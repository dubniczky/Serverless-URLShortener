...
ddb = boto3.resource('dynamodb', region_name = 'us-west-2').Table('urlshortener_links') #Set region and Dynamo DB table
...
def check_id(short_id):
    if 'Item' in ddb.get_item(Key={'id': short_id}):
...
def lambda_handler(event, context):
    ...
    body_new = '{"url":"' +short_url+'","long_url":"'+long_url+'"}'
